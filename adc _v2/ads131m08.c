...
{
    uint8_t tx[4] = { (uint8_t)(0x40u | (addr & 0x3F)), 0x00u, (uint8_t)(value >> 8), (uint8_t)(value & 0xFF) };
    uint8_t rx[4];
    ADS131_CS_LOW(h);
...
HAL_StatusTypeDef ADS131M08_ReadRegister(ADS131_Handle_t *h, uint8_t addr, uint16_t *value)
{
    ...
    uint8_t rx[2];
    ADS131_CS_LOW(h);
    if (HAL_SPI_Transmit(h->hspi, tx, 2, HAL_MAX_DELAY) != HAL_OK) { ADS131_CS_HIGH(h); return HAL_ERROR; }
    if (HAL_SPI_Receive(h->hspi, rx, 2, HAL_MAX_DELAY) != HAL_OK) { ADS131_CS_HIGH(h); return HAL_ERROR; }
    ADS131_CS_HIGH(h);
...
HAL_StatusTypeDef ADS131M08_Init(ADS131_Handle_t *h)
{
    ...
    ADS131_CS_HIGH(h);
    ADS131_RESET_LOW(h); HAL_Delay(10);
    ADS131_RESET_HIGH(h); HAL_Delay(10);
    ...
    uint16_t mode_reg = (1u << 8) | (5u << 2) | (1u << 0); /* 24-bit, OSR=1024, chop=1 */
    ...
    if (h->crc_enable) cfg_reg |= ADS131M08_CFG_CRCEN_MASK;
    ...
    for (uint8_t ch = 0; ch < ADS131M08_NUM_CHANNELS; ++ch) {
        if (ADS131M08_WriteRegister(h, ADS131M08_REG_CHn_CFG(ch), ADS131M08_CHn_CFG_ENABLE_MASK) != HAL_OK)
            return HAL_ERROR;
    }
...
HAL_StatusTypeDef ADS131M08_ReadDataFrame(ADS131_Handle_t *h, int32_t samples[ADS131M08_NUM_CHANNELS])
{
    ...
    if (h->crc_enable) frame_len += 2;

    ADS131_CS_LOW(h);
    if (spi_txrx(h, tx, rx, frame_len) != HAL_OK) { ADS131_CS_HIGH(h); return HAL_ERROR; }
    ADS131_CS_HIGH(h);

    if (h->crc_enable) {
        uint16_t received_crc = ((uint16_t)rx[frame_len - 2] << 8) | rx[frame_len - 1];
        uint16_t calc_crc = ads131_crc16(rx, frame_len - 2);
        if (received_crc != calc_crc) return HAL_ERROR;
    }
