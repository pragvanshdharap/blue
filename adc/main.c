/* main.c
 * Example for two ADS131M08 instances (SPI1 & SPI2) on STM32F7.
 */
...
/* HAL handles (CubeMX generates actual defs) */
SPI_HandleTypeDef hspi1;
SPI_HandleTypeDef hspi2;
UART_HandleTypeDef huart1;

/* ADS handles (adjust GPIO pins for your wiring) */
...
int main(void)
{
    ...
    if (ADS131M08_Init(&ads1) == HAL_OK) debug_print("ADS1 init OK\r\n");
    else debug_print("ADS1 init FAIL\r\n");

    if (ADS131M08_Init(&ads2) == HAL_OK) debug_print("ADS2 init OK\r\n");
...
/* CubeMX init stubs (implement in CubeIDE) */
void MX_SPI1_Init(void) { /* configure SPI1 (mode 3, prescaler, etc.) */ }
void MX_SPI2_Init(void) { /* configure SPI2 same as SPI1 */ }
void MX_USART1_UART_Init(void) { /* UART1 115200 8N1 */ }
void MX_GPIO_Init(void) { /* init CS/RESET outputs + DRDY inputs */ }
